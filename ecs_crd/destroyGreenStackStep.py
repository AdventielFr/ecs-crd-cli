...
class DestroyGreenStackStep(CanaryReleaseDeployStep):

    def __init__(self, infos, logger):
        """initializes a new instance of the class"""
        super().__init__(infos, 'Delete Green Cloudformation Stack', logger)
        self.timer  = 5
    
    def _on_execute(self):
        """operation containing the processing performed by this step"""
        try:
            if self.infos.green_infos.stack_id != None:
    ...
    def _destroy_stack(self, client):
        ...
        client.delete_stack(StackName=self.infos.green_infos.stack_id)
